...
2. qqplot():Q-Q Plot 全名是 Quantile-Quantile Plot，是一种视觉化比较两项数据的分布是否相同的方法。
 ...
 ![](https://raw.githubusercontent.com/zhanglantian2021/Tutorials/master/data/img/sta-01.png)
 - Q-Q Plot 不是只有判斷「是否為直線」或者「是否為常態分配」，即使畫出來的不是直線，還是能看出資料分配的特性，例如：
 ![](https://raw.githubusercontent.com/zhanglantian2021/Tutorials/master/data/img/sta-02.png)
...
```{r Matrix1}
#以下给出了所有原始数据的查看：
fishspeed <-read.table(file="https://raw.githubusercontent.com/zhanglantian2021/Tutorials/master/data/fishspeed.csv",header=T,row.names=NULL,sep=",")
fishspeed
#告诉R，β0一整列是1
...
### 正态分布Nomal
没有边界，有均值μ和方差σ2；有两个参数；用于连续性数据。![](https://raw.githubusercontent.com/zhanglantian2021/Tutorials/master/data/img/sta-normal.png)
<br>
...
### 伽马分布Gamma
边界时0到正无穷，但不包含0；有两个参数𝛼(shape)和𝛽(rate)，mean (μ = 𝛼 / 𝛽)variance (σ2 = 𝛼 / 𝛽2)；用于连续性的有边界的数据；当作是其他几个分布的先验(Normal, Poisson) ![](https://raw.githubusercontent.com/zhanglantian2021/Tutorials/master/data/img/sta-gamma.png)
<br>
...
![](https://raw.githubusercontent.com/zhanglantian2021/Tutorials/master/data/img/sta-beta.png)
<br>
...
### 泊松分布Poisson
边界是从0到正无穷，而且包含0；而且均值等于方差，即：μ = σ2(=λ)；仅有一个参数 ；用于描述某一特定事件在某一时间或空间的发生次数。或者计数型数据，即使离散型数。![](https://raw.githubusercontent.com/zhanglantian2021/Tutorials/master/data/img/sta-possion.png)
<br>
...
### 二项式分布Binomial：
边界在0到1且包含0和1；μ = np, success或yes的概率；σ2 = μ (1 – μ) ；用于二项式数据，如按0和1编码的数据，或二项式数据的比例之和等于给定的试验次数n，即离散型数![](https://raw.githubusercontent.com/zhanglantian2021/Tutorials/master/data/img/sta-binomal.png)
<br>
...
### link-function 
在glm中有一个link function，来把响应变量转换成一个没有边界的，线性的，适合进行回归分析的新回归变量，同时它也确保估计值受制于响应的原始分布的界限。![](https://raw.githubusercontent.com/zhanglantian2021/Tutorials/master/data/img/sta-link.png)
<br>
...
<br>
2. 模型矩阵形式：
![](https://raw.githubusercontent.com/zhanglantian2021/Tutorials/master/data/img/sta-10.png)

3. 参数的估计：
- 在lm和glm中的估计参数矩阵都为$\hat{\beta}=(X'X)^{-1}X'y$，但是在lmm中不是。在lmm中把本事vector的error转变成了矩阵的形式，在协方差矩阵中，还表明了残差之间相互独立，方差同质（具体看动物模型一章）
![](https://raw.githubusercontent.com/zhanglantian2021/Tutorials/master/data/img/sta-13.png)
<br>
...
- 固定和随机效应的区别
![](https://raw.githubusercontent.com/zhanglantian2021/Tutorials/master/data/img/sta-14.png)

<br>

- 得到正确的预测值和置信区间需要明确的是：
![](https://raw.githubusercontent.com/zhanglantian2021/Tutorials/master/data/img/sta-21.png)

<br>

- 关于分布的总结（以下截图来自范欢师姐上课内容）
![](https://raw.githubusercontent.com/zhanglantian2021/Tutorials/master/data/img/sta-31.png)
![](https://raw.githubusercontent.com/zhanglantian2021/Tutorials/master/data/img/sta-32.png)
![](https://raw.githubusercontent.com/zhanglantian2021/Tutorials/master/data/img/sta-33.png)
![](https://raw.githubusercontent.com/zhanglantian2021/Tutorials/master/data/img/sta34.png)
